...
def read_data_from_file_year(directory, year):
    weather_data_list = []
    file_name = f"Murree_weather_{year}"
    for filename in os.listdir(directory):
        if file_name in filename:
            with open(os.path.join(directory, filename)) as file:
                ...
                for line in file:
                    list = line.split(",")
                    weather_data_list.append(constants.WeatherData(
                        list[0], list[1], list[2], list[3], list[7], list[8], list[9]))
    return weather_data_list


def read_data_from_file_month(directory, month, Month):
    weather_data_list = []
    ...
    month_by_name=None
    for months in Month:
        if months.value==month_name:
            month_by_name=months.name

    file_name = "Murree_weather_"+list_month[0]+"_" + month_by_name
    for filename in os.listdir(directory):
        if file_name in filename:
            with open(os.path.join(directory, filename)) as file:
                file.readline()
